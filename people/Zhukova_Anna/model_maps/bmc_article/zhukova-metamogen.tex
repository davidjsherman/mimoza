...
% Web-based
Web-based tools allowing for model visualization are also emerging.  JWS online~\cite{Snoep2003}, for example, provides a mechanism for model visualization using a force-directed layout algorithm~\cite{Tamassia:2007:HGD:1202383}. It also encounters the aforementioned issues and thus is not capable of providing a readable representation for large networks.  
...
\item \emph{Force-Directed Layout}~\cite{simonetto:inria-00605921} for all the other components. (Complexity of one iteration is $O(|V|^2 + |V ||E|)$. The number of iterations is determined by the user.)
\end{itemize}
