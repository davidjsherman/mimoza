...
As the examples of NaviCell and the Cellular Overview show, not only zoomable interface but also model decomposition are important for multi-level visualization of huge models. At the general level, the network needs to be decomposed into several meaningful modules (such as compartments, pathways). If after such a decomposition the model remains complicated (e.g. the mitochondrial compartment of the yeast consensus model\cite{Heavner12} that contains 230 reactions, a further decomposition is required. We address this issue by applying the model generalization method.
...
To illustrate the use of Mimoza compare it with other available ZUI tools we visualized the yeast consensus genome-scale metabolic network (version 5\cite{Heavner12}). The result can be found at http://mimoza.bordeaux.inria.fr/smth. 
