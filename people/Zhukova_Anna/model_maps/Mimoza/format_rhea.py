...
def main(argv=None):
	...
	while start < r_len:
		...
		toSBML(r_ids[start: min(start + step, r_len)], r_id2rsps, sbml, onto)
		iteration += 1
		start += step


def toSBML(r_ids, r2rsps, sbml, onto):
	...
	for c_id in sps:
		...
		add_annotation(species, BQB_IS, to_identifiers_org_format(c_id))
