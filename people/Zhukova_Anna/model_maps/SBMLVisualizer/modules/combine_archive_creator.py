...
def archive(path, zip_file):
	...
	with ZipFile(zip_file, 'w') as zip:
		for r, dirs, files in os.walk(path):
			for f in files:
				...
				format = extension2format[extension] if (extension in extension2format) else mimetypes.guess_type(f)[0]
				if format:
	...
	with open(manifest_file, 'w+') as manifest:
		...
		manifest.write('<content location="./manifest.xml" format="http://identifiers.org/combine.specifications/omex-manifest"/>\n')
		for location, format in location2format.iteritems():
			manifest.write('<content location="%s" format="%s"/>\n' % (os.path.join(".", location), format))
