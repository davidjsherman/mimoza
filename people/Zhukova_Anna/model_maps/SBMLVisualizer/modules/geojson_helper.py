...
def tulip2geojson(graph_full, graph, geojson_file, compartment_name):
	root = graph.getRoot()
	...
	with open(geojson_file, 'w+') as f:
		...
		f.write("var gjsn__{1} = {0}\n".format(geojson.dumps(fc).replace('"id": null', ''), compartment_name))


def graph2features(graph, level_min, level_max, n2features):
	...
	for e in graph.getEdges():
		...
		if e in n2features or (s in n2features and t in n2features):
			continue
		f = edge2feature(graph, e, i, scale, level_min, level_max, x_scale, y_scale)
		i += 1
		n2features[e] = f
	...
	for n in graph.getNodes():
		if n in n2features:
			continue
		f, bg = node2feature(graph, n, id, scale, level_min, level_max, 0, onto)
		i += (2 if bg else 1)
		n2features[n] = f
...
def node2feature(graph, n, id, scale, level_min, level_max, max_bg_level, onto):
	...
	if type_[n] in TYPE_BG:
		props = {"width": size[n].getW() * x_scale, "height": size[n].getH() * y_scale,
		         "type": type_[n], "transport": transport[n], "zoom_min": level_min,
		         "zoom_max": level_max}
		return geojson.Feature(geometry=geom, properties=props, id=id)
	...
	if type_[n] in TYPE_ENTITY:
		...
		props = {"id": root[ID][n], "name": root[NAME][n], "label": get_short_name(graph, n, onto),
		         "width": size[n].getW() * x_scale, "height": size[n].getH() * y_scale,
		         "type": type_[n], "generalized": generalized, "zoom_min": level_min,
		         "zoom_max": level_max}
		if TYPE_REACTION == type_[n]:
		...
		if generalized:
			bg_props = {"width": size[n].getW() * x_scale, "height": size[n].getH() * y_scale,
		         "type": TYPE_2_BG_TYPE[type_[n]], "transport": transport[n], "zoom_min": level_max + 1,
