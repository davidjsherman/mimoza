...
def getStoichiometry(spRef):
	result = spRef.getStoichiometry()
	if not result:
		result = spRef.getStoichiometryMath()
	if not result:
		return 1
	return result


def getReactants(reaction, stoichiometry=False):
	if stoichiometry:
		return {(speciesRef.getSpecies(), getStoichiometry(speciesRef)) for speciesRef in reaction.getListOfReactants()}
	else:
		return {speciesRef.getSpecies() for speciesRef in reaction.getListOfReactants()}


def getProducts(reaction, stoichiometry=False):
	if stoichiometry:
		return {(speciesRef.getSpecies(), getStoichiometry(speciesRef)) for speciesRef in reaction.getListOfProducts()}
	else:
...
def get_compartment(reaction, model):
