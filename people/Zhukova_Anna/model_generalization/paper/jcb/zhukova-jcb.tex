...
To aid human understanding of these complete models, a ``zoomable'' approach can be used. Different abstraction levels address different types of questions about the model. The most abstract level represents compartmentalization of the model, and focusses on such questions as: Are all the compartments present? Are they well connected by transport reactions?

The next level of abstraction goes inside each of the compartments and represents the modules they contain. The questions to be addressed on this level include: Are all the essential processes present? Is the process' structure correct? Is there any organism-specific adaptation of the structure?

The following  level represents the inner structure of each of the modules to check if all the needed species and reactions are present and if their kinetics, stoichiometry, mass balance, reversibility and constraints are correct. The model on this level is intended for computer simulation.
