...
\begin{abstract}
\section*{Background}
Genome-scale metabolic model reconstruction is a complicated process including (semi-)automatic inference of reactions participating in the organism's metabolism, followed by several iterations of network analysis and improvement. Better and better automatic model inference and analysis tools are developed, but they may still miss some reactions or add erroneous ones. That is why the human expert's analysis of the model plays an important role at all the iterations of the reconstruction process. However, the size of the genome-scale models (i.e. thousands of reactions) makes it hard for a human to analyse them.
\section*{Results}
To aid a human expert in metabolic model analysis we have developed a method for knowledge-based generalization that provides a higher-level view of a metabolic model by hiding the details while preserving its essential structure. The method groups biochemical species in the model into semantically equivalent classes and generalizes them into their common parent in the ChEBI ontology. The reactions between the same generalized species are factored together into generalized reactions.
\section*{Conclusions}
We have applied our method to several metabolic models and shown that it helps finding the peculiarities and errors, as well as facilitates model understanding and comparison.      
\end{abstract}
